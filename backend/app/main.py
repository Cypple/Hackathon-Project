...
DATA_FILE = BACKEND_DIR / "data" / "sanctioned_mplads_projects.csv"
UI_DIR = PROJECT_DIR / "frontend" / "anomaly"
...
def _load_detector():
    """
    Find the anomaly detector wherever it exists in the current project.

    This makes the backend tolerant of the earlier and modular layouts.
    """
    candidates = (
    ...
    for module_name in candidates:
        try:
            ...
            detector = getattr(module, "detect_anomalies", None)
...
def ui_home():
    if not UI_INDEX.exists():
        raise HTTPException(
            status_code=500,
            detail=f"Frontend index.html not found: {UI_INDEX}",
        )
    return FileResponse(UI_INDEX)


# Serve style.css, script.js and any other frontend files.
if UI_DIR.exists():
    app.mount(
        "/ui",
...
def root():
    return {
        "message": "MPLADS Sentinel backend is running",
        "ui": "/ui/",
        "docs": "/docs",
        "api": "/api/v1",
    }
...
# We intentionally declare the routes with /api/v1 directly.
# This avoids requiring a separate router file just to make the
...
def dashboard():
    ...
    for row in works:
        ...
        projects_by_category[category] = (
            projects_by_category.get(category, 0) + 1
        )
    ...
    repeated_count = sum(
        ...
        if item.get("anomaly_type") == "repeated_amount_pattern"
    )

    extreme_count = sum(
        1 for item in anomalies
        if item.get("anomaly_type") == "extreme_allocation"
    )
    ...
    return {
        ...
        "states": list(projects_by_state.keys()),
        "categories": list(projects_by_category.keys()),
        "projects_by_status": projects_by_status,
        "projects_by_category": projects_by_category,
        "projects_by_state": projects_by_state,

        # Explicit anomaly breakdown
        "repeated_amount_pattern": repeated_count,
        "extreme_allocation": extreme_count,
...
def _build_filters() -> dict[str, list[str]]:
    ...
    return {
        ...
        "districts": values_for("DISTRICT"),
        "constituencies": values_for("CONSTITUENCY"),
        "categories": values_for("CATEGORY"),
        "statuses": values_for("STATUS"),
    }
...
# Compatibility route for the earlier frontend version.
...
def works(
    ...
    all_works = exact_filter(all_works, "STATE", state)
    all_works = exact_filter(all_works, "DISTRICT", district)
    all_works = exact_filter(all_works, "CONSTITUENCY", constituency)
    all_works = exact_filter(all_works, "CATEGORY", category)
    all_works = exact_filter(all_works, "STATUS", status)

    total = len(all_works)
    selected = all_works[offset: offset + limit]
...
def single_work(work_id: str):
    for row in load_works():
        project_id = str(
            _first(row, "PROJECT_ID", "IDA", default="")
        ).strip()
...
def anomaly_summary():
    ...
    scores = [
        ...
        if item.get("pattern_score") is not None
    ]
    ...
    return {
        "total_anomalies": len(anomalies),
        "total": len(anomalies),
        "repeated_amount_pattern": len(repeated),
        "extreme_allocation": len(extreme),
...
def anomalies(
    ...
    results = list(get_anomalies())

    if anomaly_type:
        wanted = anomaly_type.strip().lower()
        results = [
            item
            for item in results
            if str(item.get("anomaly_type") or "").lower() == wanted
        ]
    ...
    if severity:
        ...
        results = [
            item
            for item in results
            if str(item.get("severity") or "").lower() == wanted
        ]
    ...
    return {
        ...
        "offset": offset,
        "disclaimer": (
            "Anomalies indicate statistical irregularities and require "
            "verification; they do not prove fraud."
        ),
        "anomalies": selected,
        "items": selected,
    }
...
def refresh():
    ...
    works_count = len(load_works())
    anomaly_count = len(get_anomalies())

    return {
        "status": "refreshed",
        "projects": works_count,
        "anomalies": anomaly_count,
    }


# Compatibility GET refresh, useful for simple browser testing.
